	...
	for ( int i = 0; i < OBJECT_CHIP_WIDTH_NUM; i++ ) {
		for ( int j = 0; j < OBJECT_CHIP_HEIGHT_NUM; j++ ) {
			...
			if ( _data->getObject( ox, oy ) == OBJECT_SHELL ) {
				_enemy[ GRAPH_ENEMY_SMALL ]->setRect( RECT_SHELL.tx, RECT_SHELL.ty, NORMAL_CHAR_GRAPH_SIZE, NORMAL_CHAR_GRAPH_SIZE );
				_enemy[ GRAPH_ENEMY_SMALL ]->setPos( PREVIEW_X + i * OBJECT_CHIP_SIZE - ( NORMAL_CHAR_GRAPH_SIZE / 2 ), PREVIEW_Y + j * OBJECT_CHIP_SIZE - ( NORMAL_CHAR_GRAPH_SIZE - OBJECT_CHIP_SIZE ) );
				_enemy[ GRAPH_ENEMY_SMALL ]->draw( );
			}
			if ( _data->getObject( ox, oy ) == OBJECT_WATER_GHOST ) {
				_enemy[ GRAPH_ENEMY_SMALL ]->setRect( RECT_WATER_GHOST.tx, RECT_WATER_GHOST.ty, NORMAL_CHAR_GRAPH_SIZE, NORMAL_CHAR_GRAPH_SIZE );
				_enemy[ GRAPH_ENEMY_SMALL ]->setPos( PREVIEW_X + i * OBJECT_CHIP_SIZE - ( NORMAL_CHAR_GRAPH_SIZE / 2 ), PREVIEW_Y + j * OBJECT_CHIP_SIZE - ( NORMAL_CHAR_GRAPH_SIZE - OBJECT_CHIP_SIZE ) );
				_enemy[ GRAPH_ENEMY_SMALL ]->draw( );
			}
			if ( _data->getObject( ox, oy ) == OBJECT_RAY ) {
				_enemy[ GRAPH_ENEMY_SMALL ]->setRect( RECT_RAY.tx, RECT_RAY.ty, BIG_CHAR_GRAPH_SIZE, NORMAL_CHAR_GRAPH_SIZE );
				_enemy[ GRAPH_ENEMY_SMALL ]->setPos( PREVIEW_X + i * OBJECT_CHIP_SIZE - ( BIG_CHAR_GRAPH_SIZE / 2 ), PREVIEW_Y + j * OBJECT_CHIP_SIZE - ( NORMAL_CHAR_GRAPH_SIZE - OBJECT_CHIP_SIZE ) );
				_enemy[ GRAPH_ENEMY_SMALL ]->draw( );
			}
